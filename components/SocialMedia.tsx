    ...
    return (
        <div className="h-full overflow-y-auto no-scrollbar pb-24 p-4 animate-fade-in">
            <div className="max-w-6xl mx-auto">
                <div className="bg-gray-900 p-6 rounded-[2.5rem] border border-gray-800 flex flex-col lg:flex-row gap-8 shadow-2xl">
                    {/* Controls */}
                    <div className="lg:w-1/2 space-y-6">
                        <div>
                            <h3 className="text-xl font-black text-white uppercase tracking-tighter italic">Poster Creator Studio</h3>
                            <p className="text-gray-500 text-[10px] font-bold uppercase tracking-widest mt-1">Design stories for Social Media</p>
                        </div>

                        <div className="space-y-4">
                            <div>
                                <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest block mb-2">Select from Menu</label>
                                <select 
                                    className="w-full bg-black text-lemon p-4 rounded-2xl border border-gray-800 outline-none font-bold"
                            ...
                            <div>
                                <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest block mb-2">Custom Text (Optional)</label>
                                <input 
                                    type="text" 
                                    value={customText} 
                                    onChange={e => setCustomText(e.target.value)} 
                                    className="w-full bg-black text-white p-4 rounded-2xl border border-gray-800 outline-none font-bold" 
                                    placeholder="e.g. Sunday Mega Offer!"
                                />
                            </div>

                            <div>
                                <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest block mb-2">Style Category</label>
                                <div className="flex flex-wrap gap-2">
                                    {categories.map(cat => (
                                        <button 
                                            key={cat}
                                            onClick={() => setSelectedCategory(cat)}
                                            className={`px-4 py-2 text-[10px] font-black rounded-xl border transition-all uppercase tracking-widest ${selectedCategory === cat ? 'bg-lemon text-black border-lemon' : 'bg-transparent text-gray-400 border-gray-800 hover:border-gray-500'}`}
                                        >
                                            {cat}
                                        </button>
                                    ))}
                            ...
                            <div className="pt-4 flex flex-col gap-3">
                                 <button 
                                    ...
                                    className="w-full bg-gray-800 text-white font-black py-4 rounded-2xl hover:bg-gray-700 flex items-center justify-center gap-2 uppercase text-[10px] tracking-widest active:scale-95 transition-all"
                                >
                                    Shuffle Image & Theme
                                </button>
                                 ...
                                 <div className="flex gap-2">
                                    <button 
                                        ...
                                        className="flex-1 bg-blue-600/10 text-blue-400 border border-blue-600/30 font-black py-4 rounded-2xl uppercase text-[10px] tracking-widest"
                                    >
                                        Theme: {THEMES[currentThemeIndex].name}
                                    </button>
                                    <button 
                                        onClick={handleDownload}
                                        className="flex-1 bg-lemon text-black font-black py-4 rounded-2xl hover:bg-lemon-dark flex items-center justify-center gap-2 uppercase text-[10px] tracking-widest shadow-xl shadow-lemon/10 active:scale-95 transition-all"
                                    >
                                        Download
                                    </button>
                                 </div>
                        ...
                        <div className="bg-black/50 p-6 rounded-[2rem] border border-gray-800">
                            <h4 className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-4">Linked Accounts</h4>
                            <div className="flex gap-4">
                                 {user.socialMedia?.instagram ? (
                                     <a href={`https://instagram.com/${user.socialMedia.instagram.replace('@', '')}`} target="_blank" className="text-pink-500 hover:text-pink-400 flex items-center gap-1 font-bold text-xs uppercase">
                                         Instagram
                                     </a>
                                 ) : <span className="text-gray-600 text-[10px] uppercase font-black">No Instagram</span>}
                                 
                                 {user.socialMedia?.facebook ? (
                                     <a href={user.socialMedia.facebook.startsWith('http') ? user.socialMedia.facebook : `https://${user.socialMedia.facebook}`} target="_blank" className="text-blue-500 hover:text-blue-400 flex items-center gap-1 font-bold text-xs uppercase">
                                         Facebook
                                     </a>
                                 ) : <span className="text-gray-600 text-[10px] uppercase font-black">No Facebook</span>}
                            </div>
                        </div>
                    </div>

                    {/* Preview Canvas */}
                    <div className="lg:w-1/2 flex justify-center bg-black/40 rounded-[2rem] p-6 border border-gray-800">
                        <canvas 
                            ref={canvasRef} 
                            width={540} 
                            height={960} 
                            className="w-full max-w-[320px] h-auto shadow-2xl rounded-2xl bg-white animate-scale-up"
                        />
                    </div>
                </div>
            </div>
