        ...
        <div className="space-y-6 h-full flex flex-col p-2 md:p-0 animate-fade-in overflow-hidden">
            ...
            <div className="bg-lemon text-black p-4 rounded-2xl flex justify-between items-center shrink-0 shadow-xl shadow-lemon/10">
                <div className="flex flex-col">
                    ...
                    <p className="text-[8px] font-black uppercase tracking-widest mt-1 opacity-60">Supply & Promotions Center</p>
                </div>
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar space-y-8 pb-24">
                {/* ORDER HISTORY - Prominent Yellow Box */}
                {orders.length > 0 && (
                    <div className="bg-lemon border-4 border-lemon/40 p-5 rounded-[2.5rem] space-y-4 shadow-2xl animate-fade-in mx-1">
                        <div className="flex justify-between items-center px-2">
                            ...
                            <span className="text-[8px] font-black text-black/60 uppercase">{orders.length} orders</span>
                        </div>
                        <div className="flex gap-4 overflow-x-auto no-scrollbar pb-2">
                            {orders.slice().reverse().map(order => (
                                <div key={order.id} className="min-w-[300px] bg-black p-6 rounded-[2rem] border border-black/10 shadow-xl relative overflow-hidden group">
                                        ...
                                        <div>
                                            <div className="flex justify-between items-start mb-2">
                                                <span className={`text-[8px] font-black px-2.5 py-1 rounded-full uppercase ${
                                                    ...
                                                    order.status === 'Out of Stock' ? 'bg-orange-900 text-orange-100' :
                                            ...
                                            <h4 className="text-white font-black uppercase text-base leading-tight line-clamp-1">{order.productName}</h4>
                                            <p className="text-lemon font-bold text-[11px] mt-1 uppercase tracking-widest">Qty: {order.quantity} • Total: ₹{order.price * order.quantity}</p>
                                        </div>

                                        <div className="space-y-3">
                                            {/* Delivery Date Display */}
                                            <div className="bg-white/5 border border-white/10 p-3 rounded-xl">
                                                 <p className="text-[9px] text-gray-500 font-black uppercase tracking-widest leading-none mb-1">Estimated Delivery</p>
                                                 <p className="text-white font-black text-sm uppercase tracking-tighter">
                                                     {order.deliveryDate || (order.status === 'Cancelled' ? 'Order Cancelled' : 'Awaiting Confirmation')}
                                                 </p>
                                            </div>
                                            ...
                                            <div className="flex gap-2">
                                                <button 
                                                    ...
                                                    className="flex-1 bg-gray-800 hover:bg-gray-700 text-white font-black py-3 rounded-xl text-[10px] uppercase tracking-widest flex items-center justify-center gap-1.5 transition-all active:scale-95"
                                                >
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><path d="m3 21 1.9-5.7a8.5 8.5 0 1 1 3.8 3.8z"/></svg>
                                                    Chat
                                                </button>
                                                {(order.status === 'Pending' || order.status === 'Accepted') && (
                                                    <button 
                                                        onClick={() => { if(window.confirm('Cancel this order?')) onCancelOrder(order.id); }}
                                                        className="flex-1 bg-red-900/20 hover:bg-red-900/40 border border-red-900/50 text-red-500 font-black py-3 rounded-xl text-[10px] uppercase tracking-widest transition-all active:scale-95"
                                                    >
                                                        Cancel
                                                    </button>
                                                )}
                                    ...
                                    {(order.messages || []).some(m => m.sender === 'admin') && (
                ...
                {/* PRODUCT LIST */}
                <div className="px-1 grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div key={product.id} className="bg-gray-900 border border-gray-800 rounded-[2.5rem] overflow-hidden group hover:border-lemon/30 transition-all shadow-lg flex flex-col">
                        ...
                        <div className="p-5 flex flex-col gap-4">
                            <div className="min-w-0">
                                ...
                                <p className="text-gray-500 text-[10px] font-bold uppercase mt-1 line-clamp-1 italic">"{product.description}"</p>
                            </div>
                ...
                {products.length === 0 && (
                    <div className="py-24 text-center opacity-30 flex flex-col items-center">
                        ...
                        <p className="font-black uppercase text-xs tracking-[0.3em]">Marketplace is empty</p>
                    </div>
                )}
            </div>
        </div>
