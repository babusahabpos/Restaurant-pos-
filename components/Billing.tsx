...
const Billing: React.FC<BillingProps> = ({ onPrintKOT, menuItems = [], taxRate, restaurantName, isPrinterEnabled, onToggleStock }) => {
    ...
    return (
        <div className="flex flex-col h-full bg-black overflow-hidden relative">
            {/* COMPACT MENU GRID */}
            <div className="h-[50%] flex flex-col p-1 border-b border-gray-800 overflow-hidden shrink-0">
                    ...
                    {filteredMenuItems.map(item => (
                        <div key={item.id} className="relative group">
                            <div 
                                ...
                                className={`h-full p-1 rounded-md text-center cursor-pointer border transition-all flex flex-col justify-center min-h-[45px] shadow-sm ${item.inStock ? 'bg-gray-900 border-gray-800 active:bg-gray-700' : 'bg-gray-950 border-gray-900 opacity-40 grayscale'}`}
                            >
                                <p className="text-[7px] text-white font-black leading-none line-clamp-2 mb-0.5 uppercase tracking-tighter">{item.name}</p>
                                <p className="text-lemon text-[8px] font-black">₹{item.offlinePrice}</p>
                            </div>
                            <button 
                                onClick={(e) => { e.stopPropagation(); onToggleStock?.(item.id); }}
                                className="absolute -top-0.5 -right-0.5 w-3 h-3 bg-black border border-white/10 rounded-full flex items-center justify-center"
                            >
            ...
            <div className="flex-1 flex flex-col p-2 bg-gray-950 overflow-hidden pb-[100px]">
                ...
                <div className="grid grid-cols-2 gap-1 mb-1 shrink-0">
                    <input type="text" value={orderSource === 'Dine-in' ? tableNumber : customerName} onChange={e => orderSource === 'Dine-in' ? setTableNumber(e.target.value) : setCustomerName(e.target.value)} placeholder={orderSource === 'Dine-in' ? "Table" : "Name"} className="w-full bg-gray-900 text-lemon text-[9px] p-1.5 rounded border border-gray-800 outline-none font-bold" />
                    <input type="tel" value={customerPhone} onChange={e => setCustomerPhone(e.target.value)} placeholder="Phone" className="w-full bg-gray-900 text-lemon text-[9px] p-1.5 rounded border border-gray-800 outline-none font-bold" />
                </div>

                <div className="flex-1 overflow-y-auto space-y-0.5 bg-black/40 rounded-md p-1 no-scrollbar border border-gray-900">
                    {currentOrder.length === 0 ? (
                        <div className="h-full flex items-center justify-center text-lemon opacity-10 text-[8px] font-black uppercase">Cart Empty</div>
                    ) : (
                        currentOrder.map(item => (
                            <div key={item.id} className="flex items-center justify-between bg-white/5 p-1 rounded border border-white/5">
                                <div className="w-[45%] flex items-center gap-1">
                                    <button onClick={() => removeFromOrder(item.id)} className="text-red-500 scale-75">
                                ...
                                <p className="text-[9px] text-lemon font-black w-[15%] text-right">₹{(item.offlinePrice * item.quantity).toFixed(0)}</p>
                            </div>
                        ))
                    )}
                </div>
            ...
            <div className="absolute bottom-0 left-0 right-0 p-2 bg-gray-900 border-t border-gray-800 flex flex-col gap-1 z-40">
                <div className="flex items-center justify-between px-1">
                    <div className="text-right flex-1">
                        <p className="text-[14px] text-lemon font-black tracking-tighter leading-none">TOTAL ₹{total.toFixed(0)}</p>
                    </div>
                </div>
                <button 
                    onClick={handleSendToKitchen} 
                    className="w-full bg-lemon text-black font-black py-2.5 rounded-lg text-[10px] uppercase active:scale-95 disabled:opacity-20 shadow-lg"
