    ...
    return (
        <div className="flex flex-col h-full bg-black overflow-hidden relative">
            <div className="h-[45%] flex flex-col p-2 border-b border-gray-800 overflow-hidden shrink-0">
                    ...
                    <div className="flex gap-1.5 overflow-x-auto no-scrollbar shrink-0 items-center h-10">
                        {categories.map(category => (
                            <button 
                                ...
                                className={`px-4 h-8 text-[10px] font-black rounded-full whitespace-nowrap transition-all uppercase flex items-center justify-center shrink-0 ${activeCategory === category ? 'bg-lemon text-black' : 'bg-gray-800 text-lemon border border-gray-700'}`}
                            >
                                {category}
                            </button>
                        ))}
                ...
                <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-5 gap-1.5 overflow-y-auto no-scrollbar flex-1 pb-2">
                    {filteredMenuItems.map(item => (
                        <div key={item.id} className="relative">
                            <div 
                                onClick={() => addToOrder(item)} 
                                className={`h-full p-2 rounded-xl text-center cursor-pointer border transition-all flex flex-col justify-center min-h-[75px] shadow-lg ${item.inStock ? 'bg-gray-900 border-gray-800 active:bg-gray-700' : 'bg-gray-950 border-gray-900 opacity-50 grayscale'}`}
                            >
            ...
            <div className="flex-1 flex flex-col p-2 bg-gray-950 overflow-hidden pb-[120px]">
                <div className="flex items-center justify-between mb-2 shrink-0">
                    ...
                    <button onClick={resetOrder} className="text-[10px] text-red-500 font-black uppercase px-2 hover:underline">Clear</button>
                </div>

                <div className="grid grid-cols-2 gap-2 mb-2 shrink-0">
                    <input type="text" value={orderSource === 'Dine-in' ? tableNumber : customerName} onChange={e => orderSource === 'Dine-in' ? setTableNumber(e.target.value) : setCustomerName(e.target.value)} placeholder={orderSource === 'Dine-in' ? "Table #" : "Cust. Name"} className="w-full bg-gray-900 text-lemon text-[11px] p-2 rounded-lg border border-gray-800 focus:border-lemon outline-none font-bold" />
                    <input type="tel" value={customerPhone} onChange={e => setCustomerPhone(e.target.value)} placeholder="Mobile #" className="w-full bg-gray-900 text-lemon text-[11px] p-2 rounded-lg border border-gray-800 focus:border-lemon outline-none font-bold" />
                </div>

                <div className="flex-1 overflow-y-auto space-y-1 bg-black/40 rounded-xl p-1 no-scrollbar border border-gray-900">
                    {currentOrder.length === 0 ? (
                        <div className="h-full flex flex-col items-center justify-center text-lemon opacity-20 italic text-xs uppercase font-black">Empty Cart</div>
                    ) : (
                        currentOrder.map(item => (
                            <div key={item.id} className="flex items-center justify-between bg-white/5 p-2 rounded-xl border border-white/5 group">
                                ...
                                <div className="flex items-center gap-1.5">
                                    <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="w-8 h-8 flex items-center justify-center rounded-full bg-gray-800 text-white">-</button>
            ...
            <div className="absolute bottom-0 left-0 right-0 p-3 bg-gray-900 border-t border-gray-800 flex flex-col gap-2 z-40">
                <div className="flex items-center justify-between px-1">
                    ...
                    <div className="text-right">
                        <p className="text-[10px] text-gray-500 font-black uppercase leading-none mb-1">TOTAL</p>
