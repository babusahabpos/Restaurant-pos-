...
const StaffApplicationPage: React.FC<StaffApplicationPageProps> = ({ onApply, restaurantJobs = [], registeredStaff = [], onRegisterStaff, messages = [], onMarkMessageRead }) => {
    ...
    const handleCVSubmit = (e: React.FormEvent) => {
        ...
        setView('feed');
        setAppForm({ category: '', location: '', cvDetails: '' });
    };
    ...
    return (
        <div className="min-h-screen bg-black text-white font-sans flex flex-col relative overflow-hidden">
                        ...
                        {restaurantJobs.length > 0 ? restaurantJobs.slice().reverse().map(job => (
                            <div key={job.id} className="bg-gray-900 border border-gray-800 p-5 rounded-[2rem] flex justify-between items-center shadow-lg group hover:border-lemon/30 transition-colors">
                                <div className="min-w-0 pr-4">
                                    <span className="text-[8px] font-black bg-lemon text-black px-2 py-0.5 rounded-full uppercase tracking-widest">{job.category}</span>
                                    <h3 className="text-xl font-black text-white uppercase tracking-tighter mt-1 truncate">{job.restaurantName}</h3>
                                    <p className="text-lemon font-bold text-xs mt-1">Salary: ₹{job.salary}</p>
                                    <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mt-1 truncate">{job.address}</p>
                                </div>
                                <a href={`tel:${job.phone}`} className="bg-lemon text-black font-black px-6 py-4 rounded-2xl text-[10px] uppercase tracking-widest shadow-xl shadow-lemon/10 shrink-0 active:scale-95 transition-transform">Call Now</a>
                            </div>
                        )) : (
                ...
                {view === 'messages' && (
                    <div className="animate-fade-in space-y-4">
                        <div className="flex justify-between items-center mb-6">
                             <h2 className="text-2xl font-black text-white uppercase tracking-tighter">My Inbox</h2>
                             <button onClick={() => setView('feed')} className="text-gray-500 font-black text-[10px] uppercase tracking-widest bg-gray-900 px-4 py-2 rounded-xl">Close</button>
                        </div>
                        {userMessages.length > 0 ? userMessages.slice().reverse().map(msg => (
                            ...
                            <div className="py-20 text-center opacity-30 grayscale flex flex-col items-center">
                                ...
                                <p className="font-black uppercase text-xs tracking-widest">No messages received</p>
                            </div>
                        )}
                    </div>
                )}
                    ...
                    <div className="animate-fade-in">
                        {!currentUser ? (
                            <form onSubmit={handleLoginRegister} className="bg-gray-900 p-8 rounded-[2.5rem] border border-gray-800 space-y-4 shadow-2xl">
                                <h2 className="text-2xl font-black text-lemon uppercase tracking-tighter mb-2">Login / Register</h2>
                                <p className="text-xs text-gray-400 font-bold mb-6 uppercase tracking-tight">Enter your phone number to continue</p>
                                ...
                                <input placeholder="Your Full Name (New Users)" className="w-full bg-black text-lemon p-4 rounded-2xl border border-gray-800 outline-none font-bold" value={regForm.name} onChange={e => setRegForm({...regForm, name: e.target.value})} />
                                <button type="submit" className="w-full bg-lemon text-black font-black py-5 rounded-[1.5rem] uppercase tracking-widest text-xs mt-6 shadow-xl shadow-lemon/10">Continue</button>
                            </form>
                        ) : (
                            <div className="bg-gray-900 p-10 rounded-[2.5rem] border border-gray-800 text-center shadow-2xl">
                                ...
                                <p className="text-lemon font-bold text-sm tracking-widest mt-1 opacity-70">{currentUser.phone}</p>
                                <button onClick={() => { setCurrentUser(null); localStorage.removeItem('babuSahabPos_activeStaff'); }} className="text-[10px] font-black text-red-500 uppercase mt-12 hover:underline tracking-widest block mx-auto">Sign Out Account</button>
                            </div>
                        )}
                    </div>
                )}
                    ...
                    <div className="animate-fade-in">
                        {currentUser ? (
                            <form onSubmit={handleCVSubmit} className="bg-gray-900 p-8 rounded-[2.5rem] border border-gray-800 space-y-4 shadow-2xl">
                                <h2 className="text-2xl font-black text-lemon uppercase tracking-tighter mb-4">Post My Profile</h2>
                                <select className="w-full bg-black text-lemon p-4 rounded-2xl border border-gray-800 outline-none font-bold" value={appForm.category} onChange={e => setAppForm({...appForm, category: e.target.value})} required>
                                    <option value="">Choose My Role</option>
                                    <option>Head Chef</option><option>Assistant Chef</option><option>Waiter / Captain</option><option>Manager</option><option>Receptionist</option><option>Housekeeping</option><option>Delivery Rider</option>
                                </select>
                                <input placeholder="My City / Area" className="w-full bg-black text-lemon p-4 rounded-2xl border border-gray-800 outline-none font-bold" value={appForm.location} onChange={e => setAppForm({...appForm, location: e.target.value})} required />
                                <textarea placeholder="Experience summary, previous hotels, and expected salary range..." className="w-full bg-black text-lemon p-4 rounded-2xl border border-gray-800 outline-none font-bold h-40" value={appForm.cvDetails} onChange={e => setAppForm({...appForm, cvDetails: e.target.value})} required />
                                <button type="submit" className="w-full bg-lemon text-black font-black py-5 rounded-[1.5rem] uppercase tracking-widest text-xs shadow-xl shadow-lemon/20 active:scale-95 transition-transform">Publish Profile Now</button>
                                <p className="text-center text-[9px] text-gray-500 uppercase font-black mt-4 tracking-widest">Your profile will be visible to all restaurant owners instantly.</p>
                            </form>
                        ) : (
                            <div className="bg-gray-900 p-10 rounded-[2.5rem] border border-gray-800 text-center shadow-2xl">
                                <h2 className="text-xl font-black text-white uppercase mb-4 tracking-tighter">Identity Required</h2>
                                <p className="text-gray-500 text-xs font-bold uppercase mb-6">Please sign in to post your worker profile</p>
            ...
            <nav className="fixed bottom-0 left-0 right-0 bg-black border-t border-gray-800 h-20 px-4 flex justify-around items-center z-[100] backdrop-blur-lg">
                <button onClick={() => setView('feed')} className={`flex flex-col items-center gap-1 transition-colors ${view === 'feed' ? 'text-lemon' : 'text-gray-600'}`}>
                    <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><rect width="18" height="18" x="3" y="3" rx="2"/><path d="M3 9h18"/></svg>
                ...
                <div className="relative">
                    <button onClick={() => setView('apply')} className="flex items-center justify-center w-14 h-14 -mt-10 rounded-full bg-lemon text-black shadow-2xl border-4 border-black active:scale-90 transition-transform">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>
                    </button>
                </div>
                <button onClick={() => setView('account')} className={`flex flex-col items-center gap-1 transition-colors ${view === 'account' ? 'text-lemon' : 'text-gray-600'}`}>
                    <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><path d="M16 12l-4 4-4-4"/></svg>
