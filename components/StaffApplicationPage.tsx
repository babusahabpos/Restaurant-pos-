...
const StaffApplicationPage: React.FC<StaffApplicationPageProps> = ({ onApply, restaurantJobs, registeredStaff, onRegisterStaff }) => {
    ...
    const handleJobSubmit = (e: React.FormEvent) => {
        ...
        alert("CV Published to Admin & Restaurant Owners!");
        setView('feed');
    };
        ...
        <div className="min-h-screen bg-black text-white font-sans flex flex-col relative overflow-hidden">
            <header className="sticky top-0 z-20 bg-black/90 border-b border-gray-800 p-4 flex justify-between items-center backdrop-blur-md shrink-0">
                ...
                <div className="bg-lemon/10 px-3 py-1 rounded-full border border-lemon/20">
                     <span className="text-[10px] text-lemon font-black uppercase tracking-widest">{currentUser ? (currentUser.status === 'Approved' ? 'Verified Member' : 'Pending Verification') : 'Guest'}</span>
                </div>
            </header>
            ...
            <main className="flex-1 overflow-y-auto no-scrollbar p-4 pb-28">
                {view === 'feed' && (
                    <div className="space-y-4 animate-fade-in">
                        <div className="bg-gray-900 p-5 rounded-2xl border border-gray-800">
                            ...
                            <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mt-1">Direct contact available for verified members</p>
                        </div>
                            ...
                            <div key={job.id} className="bg-gray-900 border border-gray-800 p-5 rounded-2xl relative overflow-hidden">
                                ...
                                {currentUser?.status === 'Approved' ? (
                                    ...
                                    <div className="bg-gray-800/50 p-4 rounded-xl text-center">
                                        <p className="text-[10px] text-gray-500 font-bold uppercase">Account verification required to view phone</p>
                                        <button onClick={() => setView('account')} className="text-lemon text-[10px] font-black uppercase mt-2 hover:underline">Register / Login Now</button>
                                    </div>
                                )}
                            </div>
                    ...
                    <div className="animate-fade-in">
                        {!currentUser ? (
                            <form onSubmit={handleLogin} className="bg-gray-900 p-8 rounded-3xl border border-gray-800 space-y-4">
                                ...
                                <button type="submit" className="w-full bg-lemon text-black font-black py-5 rounded-2xl uppercase tracking-widest text-xs shadow-xl shadow-lemon/20 mt-4">Create Free Account</button>
                            </form>
                        ) : (
                            <div className="space-y-6">
                                <div className="bg-gray-900 p-8 rounded-3xl border border-gray-800 text-center">
                                    ...
                                    <button onClick={() => setCurrentUser(null)} className="text-[9px] font-black text-red-500 uppercase mt-8 hover:underline">Switch Account</button>
                                </div>
                            </div>
                        )}
                    </div>
                ...
                {view === 'apply' && (
                    <form onSubmit={handleJobSubmit} className="bg-gray-900 p-8 rounded-3xl border border-gray-800 space-y-4 animate-fade-in">
                        ...
                        <textarea placeholder="Tell owners about your experience and expectations..." className="w-full bg-black text-lemon p-4 rounded-xl border border-gray-800 outline-none font-bold h-40" value={appForm.cvDetails} onChange={e => setAppForm({...appForm, cvDetails: e.target.value})} required />
            ...
            <nav className="fixed bottom-0 left-0 right-0 bg-black/95 border-t border-gray-800 h-20 px-4 flex justify-around items-center z-50 backdrop-blur-lg">
                <button onClick={() => setView('feed')} className={`flex flex-col items-center gap-1 transition-all ${view === 'feed' ? 'text-lemon' : 'text-gray-600'}`}>
                    ...
                    <span className="text-[8px] font-black uppercase tracking-widest">Vacancies</span>
                </button>
                ...
                <button onClick={() => setView('account')} className={`flex flex-col items-center gap-1 transition-all ${view === 'account' ? 'text-lemon' : 'text-gray-600'}`}>
                    ...
                    <span className="text-[8px] font-black uppercase tracking-widest">My Account</span>
                </button>
            </nav>
        </div>
    );
