...
const StaffApplicationPage: React.FC<StaffApplicationPageProps> = ({ onApply, restaurantJobs = [], registeredStaff = [], onRegisterStaff }) => {
    ...
    // Sync current user with latest status from registeredStaff props
    useEffect(() => {
        if (currentUser) {
    ...
    const handleLoginRegister = (e: React.FormEvent) => {
        ...
        if (existing) {
            if (existing.isBlocked) { alert("Your account has been blocked by admin."); return; }
            setCurrentUser(existing);
            localStorage.setItem('babuSahabPos_activeStaff', JSON.stringify(existing));
            setView('feed');
        } else {
            if (!regForm.name) {
                alert("Please enter your full name for new registration.");
                return;
            }
    ...
    const handleCVSubmit = (e: React.FormEvent) => {
        ...
        if (currentUser.status !== 'Approved') { alert("Verification Pending. Please wait for admin approval before posting your profile."); return; }
        ...
        alert("Profile published to Hub!");
        setView('feed');
    };
    ...
    return (
        <div className="min-h-screen bg-black text-white font-sans flex flex-col relative overflow-hidden">
            {/* Fixed Header */}
            <header className="sticky top-0 z-20 bg-black/95 border-b border-gray-800 p-4 flex justify-between items-center backdrop-blur-md shrink-0 shadow-xl">
                ...
                <div className="bg-lemon/10 px-3 py-1 rounded-full border border-lemon/20 flex items-center gap-2">
                     <div className={`w-2 h-2 rounded-full ${currentUser ? (currentUser.status === 'Approved' ? 'bg-green-500' : 'bg-yellow-500') : 'bg-gray-600'}`}></div>
                     <span className="text-[10px] text-lemon font-black uppercase tracking-widest">
                        {currentUser ? (currentUser.status === 'Approved' ? 'Verified' : 'Pending') : 'Guest'}
                     </span>
                </div>
            </header>
                ...
                {view === 'feed' && (
                    <div className="space-y-4 animate-fade-in">
                        <div className="bg-gray-900 p-5 rounded-2xl border border-gray-800 shadow-lg">
                            <h2 className="text-lg font-black uppercase text-white tracking-tighter">Available Vacancies</h2>
                            <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mt-1">Direct contact for verified workers</p>
                        </div>

                        {restaurantJobs.length > 0 ? restaurantJobs.slice().reverse().map(job => (
                            <div key={job.id} className="bg-gray-900/50 border border-gray-800 p-5 rounded-[2rem] relative overflow-hidden shadow-2xl group transition-all hover:border-lemon/30">
                                ...
                                <div className="bg-black/50 p-4 rounded-2xl border border-white/5 my-4">
                                    <p className="text-lemon font-black text-xs uppercase tracking-widest flex items-center gap-2">
                                        <span className="text-[10px] text-gray-500">Salary:</span> ₹{job.salary} / Month
                                    </p>
                                </div>
                                ...
                                {currentUser?.status === 'Approved' ? (
                                    <a 
                                        href={`tel:${job.phone}`}
                                        className="w-full py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest bg-lemon text-black shadow-lg shadow-lemon/10 flex items-center justify-center gap-2 active:scale-95 transition-transform"
                                    >
                                        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"></path></svg>
                                        CALL NOW
                                    </a>
                                ) : (
                                    <div className="bg-gray-800/30 p-4 rounded-2xl text-center border border-dashed border-gray-700">
                                        <p className="text-[10px] text-gray-500 font-bold uppercase">
                                            {currentUser ? "Verification Pending Approval" : "Login required to view contact info"}
                                        </p>
                                        {!currentUser && (
                                            <button onClick={() => setView('account')} className="text-lemon text-[10px] font-black uppercase mt-2 hover:underline">Sign In / Register</button>
                                        )}
                                    </div>
                                )}
                            </div>
                        )) : <div className="py-20 text-center text-gray-700 font-black uppercase text-[10px] tracking-widest italic opacity-50">No vacancies at the moment</div>}
                    </div>
                )}
                        ...
                        {!currentUser ? (
                            <form onSubmit={handleLoginRegister} className="bg-gray-900 p-8 rounded-[2.5rem] border border-gray-800 space-y-4 shadow-2xl">
                                ...
                                <div className="space-y-4">
                                    <input 
                                        ...
                                        type="tel"
                                        maxLength={10}
                                        className="w-full bg-black text-lemon p-4 rounded-2xl border border-gray-800 outline-none font-bold placeholder:text-gray-700 focus:border-lemon/50" 
                                        value={regForm.phone} 
                                        onChange={e => setRegForm({...regForm, phone: e.target.value})} 
                                        required 
                                    />
                                    <input 
                                        placeholder="Full Name (For New Workers)" 
                                        className="w-full bg-black text-lemon p-4 rounded-2xl border border-gray-800 outline-none font-bold placeholder:text-gray-700 focus:border-lemon/50" 
                                        value={regForm.name} 
                            ...
                            <div className="space-y-6">
                                <div className="bg-gray-900 p-8 rounded-[2.5rem] border border-gray-800 text-center shadow-2xl">
                                    <div className="w-20 h-20 bg-gray-800 rounded-full mx-auto mb-4 flex items-center justify-center border border-gray-700 text-lemon shadow-inner">
                                    ...
                                    <div className={`mt-8 p-5 rounded-[1.5rem] border ${currentUser.status === 'Approved' ? 'bg-green-600/10 border-green-600/30' : 'bg-yellow-600/10 border-yellow-600/30'}`}>
                                        ...
                                        {currentUser.status !== 'Approved' && <p className="text-[9px] text-gray-500 uppercase mt-2 font-bold leading-relaxed">Admin will verify your identity.<br/>Takes usually 1-2 hours.</p>}
                                    </div>

                                    <button 
                                        onClick={() => { setCurrentUser(null); localStorage.removeItem('babuSahabPos_activeStaff'); }} 
                                        className="text-[10px] font-black text-red-500 uppercase mt-10 hover:underline tracking-widest"
                                    >
                                        Sign Out
                                    </button>
                                </div>
                    ...
                    <div className="animate-fade-in">
                        {currentUser?.status === 'Approved' ? (
                            <form onSubmit={handleCVSubmit} className="bg-gray-900 p-8 rounded-[2.5rem] border border-gray-800 space-y-4 shadow-2xl">
                                ...
                                <select className="w-full bg-black text-lemon p-4 rounded-2xl border border-gray-800 outline-none font-bold focus:border-lemon/50" value={appForm.category} onChange={e => setAppForm({...appForm, category: e.target.value})} required>
                                ...
                                <input placeholder="Location (City/Area)" className="w-full bg-black text-lemon p-4 rounded-2xl border border-gray-800 outline-none font-bold focus:border-lemon/50" value={appForm.location} onChange={e => setAppForm({...appForm, location: e.target.value})} required />
                                <textarea placeholder="Describe your experience & expected salary..." className="w-full bg-black text-lemon p-4 rounded-2xl border border-gray-800 outline-none font-bold h-40 focus:border-lemon/50" value={appForm.cvDetails} onChange={e => setAppForm({...appForm, cvDetails: e.target.value})} required />
                            ...
                            <div className="bg-gray-900 p-10 rounded-[2.5rem] border border-gray-800 text-center shadow-2xl">
                                 <div className="bg-gray-800 w-16 h-16 rounded-full mx-auto mb-6 flex items-center justify-center text-lemon">
                                     <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"></rect><path d="M7 11V7a5 5 0 0 1 10 0v4"></path></svg>
                                 </div>
                                 <h2 className="text-xl font-black text-white uppercase mb-4 tracking-tighter">Verified Access Only</h2>
                                 <p className="text-gray-400 text-sm mb-8 uppercase font-bold tracking-tight">You must have an approved worker account to publish your CV.</p>
            ...
            <nav className="fixed bottom-0 left-0 right-0 bg-black/95 border-t border-gray-800 h-20 px-4 flex justify-around items-center z-[100] backdrop-blur-lg shadow-[0_-10px_20px_rgba(0,0,0,0.5)]">
                <button onClick={() => setView('feed')} className={`flex flex-col items-center gap-1 transition-all ${view === 'feed' ? 'text-lemon' : 'text-gray-600'}`}>
                    ...
                    <span className="text-[8px] font-black uppercase tracking-widest">Job Feed</span>
                </button>

                <div className="relative">
                    <button onClick={() => setView('apply')} className="flex items-center justify-center w-16 h-16 -mt-10 rounded-full bg-lemon text-black shadow-2xl shadow-lemon/30 active:scale-90 transition-transform border-4 border-black">
