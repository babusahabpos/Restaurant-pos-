    ...
    return (
        <div className="space-y-6">
            <div className="flex gap-2 bg-gray-900 p-2 rounded-2xl border border-gray-800">
                ...
                >
                    Inbox ({applications.filter(a => !a.isRead).length})
                </button>
                <button 
                ...
                >
                    Live Vacancies ({activeJobs.length})
                </button>
            </div>
            ...
            {view === 'inbox' && (
                <div className="bg-gray-900/50 p-6 rounded-3xl border border-gray-800">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {applications.slice().reverse().map(app => (
                                <div key={app.id} className="bg-black p-5 rounded-2xl border border-gray-800 flex flex-col justify-between relative overflow-hidden group">
                                    {!app.isRead && <span className="absolute top-2 right-2 w-2 h-2 bg-lemon rounded-full"></span>}
                                    <div>
                    ...
                    ) : <p className="text-center py-20 text-gray-600 font-black uppercase text-[10px]">No staff profiles yet</p>}
                </div>
            )}

            {view === 'create' && (
                <div className="bg-gray-900 p-8 rounded-3xl border border-gray-800">
                    <h3 className="text-xl font-black text-white uppercase mb-6 tracking-tighter">Create Vacancy</h3>
                    <form onSubmit={handleJobSubmit} className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <input placeholder="Category (e.g. Chef, Waiter)" className="bg-black text-lemon p-4 rounded-xl border border-gray-800 outline-none font-bold" value={jobForm.category} onChange={e => setJobForm({...jobForm, category: e.target.value})} required />
                        </div>
                        <input placeholder="Exact Address" className="w-full bg-black text-lemon p-4 rounded-xl border border-gray-800 outline-none font-bold" value={jobForm.address} onChange={e => setJobForm({...jobForm, address: e.target.value})} required />
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <input placeholder="Monthly Salary" className="bg-black text-lemon p-4 rounded-xl border border-gray-800 outline-none font-bold" value={jobForm.salary} onChange={e => setJobForm({...jobForm, salary: e.target.value})} required />
            ...
            {view === 'active' && (
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        )) : <p className="col-span-full text-center py-20 text-gray-700 font-black uppercase text-[10px] tracking-widest">No vacancies matching your search</p>}
                    </div>
                </div>
            )}
        </div>
