...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ users, staffUsers, tickets, staffRequests, staffApplications, onApproveReject }) => {
    ...
    const activityItems = [
        ...pendingUsers.map(u => ({ type: 'User Registration', title: u.restaurantName, subtitle: `Owner: ${u.name}`, time: 'Awaiting Approval' })),
        ...pendingStaffSubs.map(u => ({ type: 'Staff Subscription', title: u.name, subtitle: `Contact: ${u.phone}`, time: 'Payment Pending' })),
        ...tickets.filter(t => t.status === 'Open').map(t => ({ type: 'Support Ticket', title: t.subject, subtitle: `From: ${t.userName}`, time: 'Urgent' })),
        ...staffRequests.filter(r => !r.isRead).map(r => ({ type: 'Staff Requirement', title: r.requirement, subtitle: r.restaurantName, time: 'New' })),
        ...staffApplications.filter(a => !a.isRead).map(a => ({ type: 'Staff Application', title: a.staffName, subtitle: a.category, time: 'New' })),
    ];
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="bg-gray-900 border border-gray-800 p-5 rounded-2xl">
                    <p className="text-[10px] text-gray-500 font-black uppercase tracking-widest">Pending Subs</p>
                    <p className="text-3xl font-black text-green-500 mt-1">{pendingStaffSubs.length}</p>
                </div>
            </div>
                    ...
                    <div className="flex-1 overflow-y-auto no-scrollbar space-y-4">
                        {pendingUsers.length > 0 ? (
                            ...
                            <div className="h-full flex items-center justify-center">
                                <p className="text-gray-700 font-black uppercase text-xs">No pending restaurant approvals</p>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="p-6 bg-gray-900 rounded-3xl border border-gray-800 flex flex-col h-[500px]">
                    ...
                    <div className="flex-1 overflow-y-auto no-scrollbar space-y-3">
                        {activityItems.length > 0 ? activityItems.map((item, idx) => (
                            <div key={idx} className="bg-black/50 p-4 rounded-2xl border border-gray-800 flex justify-between items-center group hover:border-lemon/30 transition-all">
                                <div>
                                    ...
                                    <p className="text-white font-bold uppercase text-xs mt-1">{item.title}</p>
                                    <p className="text-[9px] text-gray-500 font-bold uppercase mt-0.5">{item.subtitle}</p>
                                </div>
                                <div className="text-right">
                                    <p className="text-[9px] font-black text-gray-400 uppercase tracking-tighter">{item.time}</p>
                                </div>
                            </div>
                        )) : (
                            <div className="h-full flex items-center justify-center">
                                <p className="text-gray-700 font-black uppercase text-xs">All caught up! No new activity.</p>
                            </div>
                        )}
                    </div>
                </div>
