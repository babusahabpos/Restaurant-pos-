...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ users, staffUsers, tickets, staffRequests, staffApplications, onApproveReject }) => {
    ...
    // Aggregate activity feed from all channels for a centralized view
    const activityItems = [
        ...pendingUsers.map(u => ({ type: 'Restaurant', title: u.restaurantName, subtitle: `Owner: ${u.name}`, time: 'Pending Approval', status: 'priority' })),
        ...pendingStaffSubs.map(u => ({ type: 'Worker Sub', title: u.name, subtitle: `Contact: ${u.phone}`, time: 'Awaiting Pay', status: 'priority' })),
        ...
        ...staffApplications.filter(a => !a.isRead).map(a => ({ type: 'Worker CV', title: a.staffName, subtitle: a.category, time: 'New Submission', status: 'normal' })),
        ...
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-gray-900 border border-gray-800 p-5 rounded-2xl">
                    <p className="text-[10px] text-gray-500 font-black uppercase tracking-widest">Live Restaurants</p>
                    <p className="text-3xl font-black text-white mt-1">{users.filter(u => u.status === UserStatus.Approved).length}</p>
                </div>
                <div className="bg-gray-900 border border-gray-800 p-5 rounded-2xl">
                    <p className="text-[10px] text-gray-500 font-black uppercase tracking-widest">Staff Members</p>
                    <p className="text-3xl font-black text-lemon mt-1">{staffUsers.length}</p>
                </div>
                <div className="bg-gray-900 border border-gray-800 p-5 rounded-2xl">
                    <p className="text-[10px] text-gray-500 font-black uppercase tracking-widest">Support Load</p>
                    <p className="text-3xl font-black text-blue-500 mt-1">{tickets.filter(t => t.status === 'Open').length}</p>
                </div>
                <div className="bg-gray-900 border border-gray-800 p-5 rounded-2xl">
                    <p className="text-[10px] text-gray-500 font-black uppercase tracking-widest">Total Alerts</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="flex-1 overflow-y-auto no-scrollbar space-y-4">
                        {pendingUsers.length > 0 ? (
                            ...
                            <div className="h-full flex items-center justify-center opacity-30 grayscale">
                                <p className="text-gray-500 font-black uppercase text-xs tracking-widest">No pending registrations</p>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="p-6 bg-gray-900 rounded-3xl border border-gray-800 flex flex-col h-[500px]">
                    <h3 className="mb-6 text-xl font-black text-white uppercase tracking-tight">Live Activity Feed</h3>
                    <div className="flex-1 overflow-y-auto no-scrollbar space-y-3">
                        {activityItems.length > 0 ? activityItems.map((item, idx) => (
                            ...
                            <div className="h-full flex items-center justify-center opacity-30 grayscale">
                                <p className="text-gray-500 font-black uppercase text-xs tracking-widest">System quiet</p>
                            </div>
                        )}
                    </div>
                </div>
