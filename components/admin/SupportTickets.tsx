                   ...
                   {ticket.messages.map((msg, index) => (
                       <div key={index} className={`flex ${msg.sender === 'admin' ? 'justify-end' : 'justify-start'}`}>
                           <div className={`max-w-md p-3 rounded-lg ${msg.sender === 'admin' ? 'bg-lemon/20' : 'bg-gray-700'}`}>
                               ...
                               {/* RENDERING ATTACHMENTS FOR ADMIN */}
                               {msg.attachment && (
                                    <div className="mt-3 border-t border-gray-600 pt-2">
                                        {msg.attachmentType === 'image' ? (
                                            <a href={msg.attachment} target="_blank" rel="noopener noreferrer">
                                                <img src={msg.attachment} alt="Attachment" className="max-h-48 rounded border border-gray-500 hover:opacity-90 transition-opacity cursor-pointer" />
                                                <p className="text-[10px] text-blue-400 mt-1">View Full Size</p>
                                            </a>
                                        ) : (
                                            <a href={msg.attachment} download="attachment.pdf" className="flex items-center gap-2 bg-black/40 p-2 rounded text-blue-400 hover:bg-black/60 transition-colors">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14.5 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7.5L14.5 2z"/><polyline points="14 2 14 8 20 8"/></svg>
                                                <span className="text-xs font-bold">Download PDF Attachment</span>
                                            </a>
                                        )}
                                    </div>
                               )}
