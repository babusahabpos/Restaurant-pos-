...
const OnlineOrders: React.FC<OnlineOrdersProps> = ({ onPrintKOT, menuItems = [] }) => {
    ...
    return (
        <div className="flex flex-col h-full bg-black overflow-hidden relative">
            {/* GRID SECTION */}
            <div className="h-[55%] flex flex-col p-1 border-b border-gray-800 overflow-hidden shrink-0">
                ...
                <div className="grid grid-cols-4 sm:grid-cols-6 lg:grid-cols-8 gap-1 overflow-y-auto no-scrollbar content-start pb-1">
                    {filteredMenuItems.map(item => (
                        <div key={item.id} onClick={() => addToOrder(item)} className="bg-gray-900 p-1 rounded text-center cursor-pointer border border-gray-800 active:bg-gray-700 transition-all flex flex-col justify-center h-14 shadow-sm">
                           <p className="text-[7px] text-white font-black leading-tight line-clamp-2 mb-0.5 uppercase tracking-tighter">{item.name}</p>
                           <p className="text-lemon text-[8px] font-black leading-none">₹{item.onlinePrice}</p>
                        </div>
                    ))}
                </div>
            </div>
            ...
            <div className="flex-1 flex flex-col p-1.5 bg-gray-950 overflow-hidden pb-[60px]">
                <div className="flex gap-1 mb-1 shrink-0">
                    <select value={platform} onChange={e => setPlatform(e.target.value as 'Swiggy' | 'Zomato')} className="flex-1 bg-gray-900 text-lemon text-[9px] font-black p-1 rounded border border-gray-800 outline-none uppercase">
                        <option>Swiggy</option>
                        <option>Zomato</option>
                    </select>
                    <input type="text" placeholder="ID" value={orderId} onChange={e => setOrderId(e.target.value)} className="flex-1 bg-gray-900 text-lemon text-[9px] font-bold p-1 px-2 rounded border border-gray-800 outline-none uppercase" />
                </div>

                <div className="flex-1 overflow-y-auto space-y-0.5 bg-black/40 rounded p-1 no-scrollbar border border-gray-900">
                    {currentOrder.length === 0 ? (
                        <div className="h-full flex items-center justify-center text-lemon opacity-10 text-[9px] font-black uppercase">Cart is Empty</div>
                    ) : (
                        currentOrder.map(item => (
                            <div key={item.id} className="flex items-center justify-between bg-white/5 p-1 rounded border border-white/5">
                                ...
                                <p className="text-[10px] text-lemon font-black min-w-[25px] text-right">₹{(item.onlinePrice * item.quantity).toFixed(0)}</p>
                            </div>
                        ))
                    )}
                </div>
            ...
            <div className="absolute bottom-0 left-0 right-0 p-2 bg-gray-900 border-t border-gray-800 flex flex-col gap-1 z-40">
                <p className="text-right px-1 text-lemon font-black text-[10px] uppercase">Online Total: ₹{total.toFixed(0)}</p>
                <button 
