...
const OnlineOrders: React.FC<OnlineOrdersProps> = ({ onPrintKOT, menuItems = [] }) => {
    ...
    return (
        <div className="flex flex-col h-full bg-black overflow-hidden relative">
            <div className="h-[42%] flex flex-col p-1 border-b border-gray-800 overflow-hidden shrink-0">
                ...
                <div className="grid grid-cols-5 sm:grid-cols-7 lg:grid-cols-9 gap-1 overflow-y-auto no-scrollbar flex-1 pb-1">
                    {filteredMenuItems.map(item => (
                        <div key={item.id} onClick={() => addToOrder(item)} className="bg-gray-900 p-0.5 rounded text-center cursor-pointer border border-gray-800 active:bg-gray-700 transition-all flex flex-col justify-center min-h-[35px] shadow-sm">
                           <p className="text-[6px] text-white font-black leading-none line-clamp-2 mb-0.5 uppercase tracking-tighter">{item.name}</p>
                           <p className="text-lemon text-[7px] font-black">₹{item.onlinePrice}</p>
                        </div>
                    ))}
                </div>
            </div>
            ...
            <div className="flex-1 flex flex-col p-2 bg-gray-950 overflow-hidden pb-[80px]">
                <div className="flex gap-1 mb-1 shrink-0">
                    <select value={platform} onChange={e => setPlatform(e.target.value as 'Swiggy' | 'Zomato')} className="flex-1 bg-gray-900 text-lemon text-[8px] font-black p-1 rounded border border-gray-800 outline-none uppercase">
                        <option>Swiggy</option>
                        <option>Zomato</option>
                    </select>
                    <input type="text" placeholder="ID" value={orderId} onChange={e => setOrderId(e.target.value)} className="flex-1 bg-gray-900 text-lemon text-[8px] font-bold p-1 rounded border border-gray-800 outline-none uppercase" />
                </div>

                <div className="flex-1 overflow-y-auto space-y-0.5 bg-black/40 rounded-md p-1 no-scrollbar border border-gray-900">
                    {currentOrder.length === 0 ? (
                        <div className="h-full flex items-center justify-center text-lemon opacity-10 text-[7px] font-black">Empty</div>
                    ) : (
                        currentOrder.map(item => (
                            <div key={item.id} className="flex items-center justify-between bg-white/5 p-0.5 rounded border border-white/5">
                                ...
                                <div className="flex items-center gap-1">
                                    ...
                                    <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="w-4 h-4 rounded bg-gray-800 text-white text-[7px]">+</button>
                                </div>
                                <p className="text-[8px] text-lemon font-black min-w-[20px] text-right">₹{(item.onlinePrice * item.quantity).toFixed(0)}</p>
                            </div>
                        ))
                    )}
                </div>
            ...
            <div className="absolute bottom-0 left-0 right-0 p-1.5 bg-gray-900 border-t border-gray-800 flex flex-col gap-1 z-40">
                <p className="text-center text-lemon font-black text-[10px]">TOTAL: ₹{total.toFixed(0)}</p>
                <button 
                    onClick={handleSendToKitchen} 
                    className="w-full bg-lemon text-black font-black py-2 rounded-lg text-[8px] uppercase active:scale-95 disabled:opacity-20"
